...
## Figure 3b-d & Extended Data Figs. 7-8 - Plot completeness, contamination, number of contigs, and contigs N50 of the bins
```{r}
...
## Figure 3e & Extended Data Figs. 7-8 - Plot terminal damage levels per bin
```{r}
## Figure 3e 
...
## Figure 3f-g & Extended Data Figs. 7-8 - Plot genus and species assignment for the bins
```{r}
...
## Figure 5b & Supplementary Notes - Bar plots for pathway-taxonomy
```{r}
...
## Extended Data Figure 6a - MetaPhlAn2 jaccard distance heatmap for subset with short reads and subset with long reads 
```{r heatmap}
...
## Extended Data Figure 6a - Pathway jaccard distance heatmap for subset with short reads and subset with long reads 
```{r heatmap}
...
## Supplementary Notes - Visualize number of reads aligned for metaphlan results
```{r}
...
## Supplementary Notes - Bacteroidetes vs. Firmicutes terminal damage levels
```{r}
...
## Supplementary Notes - Boxplots for prokka gene alignment
```{r}
all_samples_family <- read.table("/Users/marshacw/Downloads/for_boxplot.txt",sep="\t",header=T, stringsAsFactors = F, quote="", row.names=1)

md= read.table("/Users/marshacw/Documents/Kostic Lab/Ancient Microbiome/filtered_full/prokka_alignment_20200705/for_heatmap_metadata.txt",sep="\t",header=T)
md <- md[md$SampleID %in% colnames(all_samples_family) , ]
all_samples_family <- all_samples_family[, colnames(all_samples_family) %in% md$SampleID]
all_samples_family <- all_samples_family[, match(md$SampleID, colnames(all_samples_family))]
all(colnames(all_samples_family) == md$SampleID) # check that otu table and mapping file has same order

md$SampleName <- factor(md$SampleName, levels = c("Paleofeces","Non-urban","Urban"))

all_samples_family <- setDT(all_samples_family, keep.rownames=TRUE)
all_samples_family_long <- gather(all_samples_family, sampleID, relab, -rn)
all_samples_family_long$SampleType <- md$SampleName[match(all_samples_family_long$sampleID,md$SampleID)]

all_samples_family_long$SampleType <- factor(all_samples_family_long$SampleType, levels = c("Paleofeces","Non-urban", "Urban"))

tiff(file="/Users/marshacw/Documents/Kostic Lab/Ancient Microbiome/filtered_full/prokka_alignment_20200705/Fig_boxplot8.tiff", res=300, pointsize=0.5, width=2300, height=2500)
ggplot(all_samples_family_long, aes(x=SampleType, y=relab, fill=SampleType))+
  geom_boxplot(outlier.shape=NA, lwd=0.25) +
  geom_point(color="black", size = 1.3, shape=21, position = position_jitterdodge(), stroke=0.1, alpha=0.75)+
  labs(fill="Sample type", y="Relative abundance") +
  scale_fill_manual(values = c("#ff7a7a", "#8860b3", "#62a9d9"))+
  theme_classic() +
  theme(legend.key.size = unit(0.5, "cm"), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(), strip.text.x = element_text(size = 7), axis.text.y = element_text(size = 6), axis.title.y=element_text(size=8), legend.text=element_text(size=8), strip.background.x = element_blank(), legend.title=element_text(size=8), axis.line = element_line(size = 0.2), axis.ticks = element_line(size=0.1)) +
  facet_wrap(.~rn, scales="free_y", ncol=3) +
...
## Supplementary Notes - Heatmap of UDG vs non-UDG jaccard distance at species level (MetaPhlAn2 output)
```{r heatmap}
...
## Supplementary Notes - Heatmap of UDG vs non-UDG jaccard distance at 0.9 or 0.5 protein level.
```{r heatmap}
...
## Supplementary Notes - MetaPhlAn2 tSNE for UDG vs. non-UDG samples
```{r}
...
ggplot(data=md, aes(x=tsne1, y=tsne2, color=SampleType)) + geom_point(size=5) + xlab("SNE1") + ylab("SNE2") + scale_x_continuous(breaks=c()) + scale_y_continuous(breaks=c()) + theme_bw() + coord_equal() + labs(color="Sample type") +
  geom_text(aes(label=SampleID), hjust=0.6, vjust=1.5, size=8) + 
  theme(text=element_text(size=28),axis.title.x = element_text(size=28), axis.title.y=element_text(size=28), legend.text=element_text(size=26), legend.title = element_text(size=26), legend.key = element_rect(size = 12), legend.key.size = unit(8, 'lines')) 
dev.off()
```
